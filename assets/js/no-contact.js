...
if (localStorage.getItem("theme") == "dark") {
    ...
    document
        .querySelectorAll(".theme-switcher")
        .forEach((theme) => theme.classList.add("light-mode"));
}
...
document.querySelectorAll(".theme-switcher").forEach((item) =>
    item.addEventListener("click", () => {
        ...
        document
            .querySelectorAll(".theme-switcher")
            .forEach((theme) => theme.classList.toggle("light-mode"));
    })
);
    ...
    for (let i = 0; i < headerItems; i++) {
        if (i === 0) {
            if (windowPos < divPos[1].pos - 80) {
                document
                    .querySelectorAll(`[data-header="${divPos[0].name}"]`)
            ...
            if (windowPos >= divPos[headerItems - 1].pos - 80) {
                document
                    .querySelectorAll(
                        `[data-header="${divPos[headerItems - 1].name}"]`
                    )
                    .forEach((i) => i.classList.add("active"));
            } else {
                document
            ...
            if (
                windowPos >= divPos[i].pos - 80 &&
                windowPos < divPos[i + 1].pos - 80
            ) {
                document
                    .querySelectorAll(`[data-header="${divPos[i].name}"]`)
                    .forEach((i) => i.classList.add("active"));
            } else {
                document
                    .querySelectorAll(`[data-header="${divPos[i].name}"]`)
...
function InfoWrite() {
    ...
    document.querySelector(
        '[data-item="address"]'
    ).innerHTML = `${personal.address.city.long}, ${personal.address.state.short}`;
...
function typeWriter() {
  ...
  `,
    ...
    document.querySelector(".about-desc .about").innerHTML =
        msg[0].substring(0, txtPos) + '<span class="blink">▮</span>';
...
function allStacksWrite() {
    for (let i = 0; i < skills.hard.length; i++) {
        document.querySelector(
            ".all-stacks-display"
        ).innerHTML += `<div class="stack"><img src="./src/img/stacks/${skills.hard[i]}.svg" alt="${skills.hard[i]}" title="${skills.hard[i]}">${skills.hard[i]}</div>`;
    }
}
...
function portfolioWrite() {
    ...
    <div class="grid-item" data-animation="bottom">
      <img src="./media/sites/${
          portfolio[i].img ? portfolio[i].img : "skeleton"
      }.png" alt="Imagem" class="grid-item-img">
      <div class="grid-item-container">
        ...
        <div class="grid-item-buttons">
          <a href="${
              portfolio[i].site
                  ? portfolio[i].site
                  : "https://github.com/YaGRRusso"
          }" target="_blank"><img src="./src/img/social/site.svg" alt="site">Visitar</a>
        </div>
      </div>
...
document
    .querySelector(".portfolio-modal span")
...
function footerWrite() {
    document.querySelector(
        '.footer-contact span[data-contact="address"]'
